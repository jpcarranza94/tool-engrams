...
- **Narrow-or-archive candidates** — memories with `q<0.5` AND noise-dominant (more `noise` than `helpful` verdicts). The surfacing gate already suppresses these, so they're dead weight until fixed. Each row shows its triggers; prefer **trigger-narrowing** over archiving unless the body itself is useless.
