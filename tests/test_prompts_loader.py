...
def test_packaged_default_is_found(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(loader, "_USER_OVERRIDE_DIR", tmp_path / "no-such-dir")
...
def test_user_override_takes_precedence_over_default(tmp_path, monkeypatch):
    override = tmp_path / "watcher.md"
    override.write_text("CUSTOM WATCHER")
    monkeypatch.setattr(loader, "_USER_OVERRIDE_DIR", tmp_path)
...
def test_env_var_takes_precedence_over_user_override(tmp_path, monkeypatch):
    user_file = tmp_path / "watcher.md"
    user_file.write_text("USER OVERRIDE")
    monkeypatch.setattr(loader, "_USER_OVERRIDE_DIR", tmp_path)
...
def test_interpolation_replaces_variables(tmp_path, monkeypatch):
    ...
    override.write_text("Hello {name}, target={target_date}")
    monkeypatch.setattr(loader, "_USER_OVERRIDE_DIR", tmp_path)
...
def test_missing_prompt_raises(monkeypatch, tmp_path):
    monkeypatch.setattr(loader, "_USER_OVERRIDE_DIR", tmp_path / "nope")
