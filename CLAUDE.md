...
- `skills/` — Claude Code skill files: `remember/`, `recall/`, `forget/` (short names for plugin namespacing `/tool-engrams:<name>`; install.sh symlinks them into ~/.claude/skills under `engram-*` names; no frontmatter `name` — naming comes from the folder/symlink basename)
- `.claude-plugin/` — plugin.json (8 hooks via the `plugin/hook.sh` shim, `defaultEnabled: false`) + marketplace.json (the repo is its own marketplace)
- `plugin/` — `hook.sh` (fail-open dispatcher: stamp-compare, exec venv engram, `{}` while dark) and `bootstrap.sh` (detached venv build under `${CLAUDE_PLUGIN_DATA}`, lock-serialized). Plugin vs install.sh are mutually exclusive; DB stays at `~/.claude/tool-engrams/` for both (ADR-0004)
