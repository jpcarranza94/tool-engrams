...
ToolEngrams is a tool-bound memory system for coding agents. Memories bind to command prefixes (e.g., `git push --force`) and surface automatically via target harness hooks when the agent is about to run a matching command.
...
- `toolengrams/` — core package
  ...
  - `consolidation/` — nightly agent (spawn Opus review, launchd schedule; the session collector moved to the target adapter)
...
7. **Doctor** (`cli/doctor.py`) — `engram doctor` checks plumbing (hooks wired in settings.json, `engram` on PATH, claude ≥ 2.1.117, DB schema, kill switch) plus liveness with zero extra writes: max `session_turns.updated_ts` = last hook fire, max `watcher_state.last_tick_ts` = last watcher tick. WARN-only (fresh-but-quiet install) exits 0; any FAIL exits 1. install.sh step 4 runs it. Related: `$ENGRAM_SURFACE_NOTICE=1` makes pretool/post-tool-failure emit a visible `systemMessage` when memories surface (smoke tests, surfacing debugging).
...
- `$ENGRAM_CODEX_WATCHER_MODEL` — model for the Codex engine's watcher calls. If unset, the Codex adapter passes no `-m` and lets `~/.codex/config.toml` choose.
- `$ENGRAM_CODEX_FORMATION_MODEL` / `$ENGRAM_CODEX_EVAL_MODEL` — Codex per-role overrides; beat `$ENGRAM_CODEX_WATCHER_MODEL` for their role.
