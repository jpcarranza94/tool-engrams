...
2. **PostToolUse hook** (`post_tool.py`) — increments the per-session turn counter and fires the recovery fast-path tick (formation + eval) on a failure→success. It **no longer credits memories**: the v9 "success = useful" bump was the bug this redesign fixes; usefulness is now judged by the evaluation watcher.
