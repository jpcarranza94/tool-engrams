...
**Tool-bound memory for coding agents.** Agent-facing tools become self-documenting through interaction: an agent fails a call, the system remembers why, and next session (or next month) arrives with that knowledge in hand. ToolEngrams supports two swappable harness roles: the **target** agent whose tool calls are hooked, and the **engine** used for background LLM work. Claude Code and Codex can be mixed against one shared SQLite DB.
...
already-running Claude Code or Codex session won't have them — and walk through
[Verify it's working](#verify-its-working).

For Codex:

```bash
./install.sh --target codex --engine codex
```

Codex uses hash-based hook trust. On the first Codex run after install, Codex
will prompt you to trust the ToolEngrams hooks; editing `~/.codex/hooks.json`
later triggers the trust prompt again.

## The problem

Coding agents are great at well-known CLIs. They're not great at:
...
Hooks talk to the selected target harness on the hot path; two background LLM roles (formation + evaluation) and a nightly agent run out-of-band through the selected engine. All targets and engines share one SQLite DB.

```
              Target agent (Claude Code or Codex)
         ┌─────────────────────────────────────┐
...
No network and no LLM on the hot path (hooks are single-digit-ms SQL lookups). Both watcher roles and consolidation are permissioned engine sessions (`claude -p` for the Claude Code engine, `codex exec` for the Codex engine) that do their job by **calling the `engram` CLI** — there is no constrained JSON schema to parse (see `docs/adr/0001`).
...
| **PostToolUseFailure hook** | `toolengrams/hooks/post_tool_failure.py` | After a real tool failure where the target has a dedicated event | Surfaces `hint`-kind memories as `additionalContext` (gated by `q`); arms the formation watcher. Codex performs the same surfacing inline on `PostToolUse` when the response is a clear failure. |
| **Formation watcher** | `toolengrams/watcher/tick.py` (role `formation`) | Fresh detached engine run per turn (Stop), recovery, correction, or session-end flush | Reads the target transcript delta (plus the tail of the prior window and the list of memories it already saved this session — ADR-0005) and calls `engram remember` for new patterns. |
| **Evaluation watcher** | `toolengrams/watcher/tick.py` (role `eval`) | Fresh detached engine run at the Stop/flush **after** a surface, only when surfaces are pending | Reads forward, judges how each surfaced memory fared (`engram judge`), and can reversibly `engram quarantine` a demonstrably harmful memory (ADR-0007). |
| **Consolidation** | `toolengrams/consolidation/agent.py` | Nightly (launchd/cron) | Engine agent reviews yesterday's wired target sessions — prunes noise, **narrows over-matching triggers**, discovers missed patterns, deduplicates. |

## Surfacing pipeline

When a target agent is about to run a tool call, this is the full path from input → injected memory. The pipeline is the same in PreToolUse and the post-failure surfacing path — just a different `kind` filter and output field.
...
| **block** | `PreToolUse` (before every whitelisted call) | **Denies** the call + injects body. The deny is invisible to the user; it fails the call for the agent and prompts an in-loop retry with context. | Rare. Only for things you actively want to prevent (destructive ops, silently-wrong output). |
...
| Source | Code | How |
|---|---|---|
| **Formation watcher** (background engine run) | `toolengrams/watcher/` + `toolengrams/prompts/defaults/watcher.md` | A permissioned session (model via engine-specific watcher env vars) whose only allowed command is `engram remember`. It reads the transcript delta and runs `engram remember …` for patterns worth keeping — native tool-calling, no JSON schema. |
| **Consolidation** (nightly engine run) | `toolengrams/consolidation/agent.py` + `toolengrams/prompts/defaults/consolidation.md` | The selected engine issues `engram remember / forget / trigger` commands directly after reviewing yesterday's wired target sessions. |
...
```
toolengrams/
├── hooks/             ← one file per target hook event
...
├── consolidation/     ← nightly engine agent (+ runs.py seam, schedule)
├── target/            ← hooked-harness adapters (claude-code, codex)
├── engine/            ← headless runner adapters (claude-code, codex)
├── prompts/
...
└── watcher/           ← event-driven formation + evaluation
    ...
    ├── agent.py               permissioned per-role engine session runner
    ├── state.py               watcher_state seam, keyed (work_session_id, role)
    ├── transcript_io.py       cursor reads; target adapter formats JSONL → delta
...
- `ENGRAM_WATCHER_MODEL=haiku` — cheaper Claude Code watcher; `ENGRAM_FORMATION_MODEL` / `ENGRAM_EVAL_MODEL` override per role. Codex uses `ENGRAM_CODEX_WATCHER_MODEL` / `ENGRAM_CODEX_FORMATION_MODEL` / `ENGRAM_CODEX_EVAL_MODEL`.
- **`engram config` / `engram engine` — durable settings, no reinstall.** The `ENGRAM_*` levers above can also live in `<engram home>/config.json`, an engine-keyed file. `engram engine set codex` switches the background engine — picked up on the next tick, no reinstall. `engram config set engines.codex.eval_model gpt-5` sets a per-engine model; `engram config show` prints every key with its effective value and source. Precedence is **explicit env > file > default** (an env var always wins), so the file is durable defaults and env is the one-off override. Switching the *engine* (background runner) is a runtime choice; the *target* harness is wired at install time — and several targets can be wired at once, so they coexist and there is nothing to switch. See `docs/adr/0012`.
- `engram monitor` — live dashboard of watcher runs with per-run engine, USD cost when available, tokens, and the decision stream.

**What is stored where.** Everything is local; nothing leaves your machine except the selected engine calls themselves (same data path as your normal Claude Code or Codex usage).
...
ToolEngrams injects stored text into the target agent's context and can deny tool calls — treat its data as part of your attack surface:

- **Memory bodies are untrusted input to future sessions.** They form autonomously from transcripts via a background LLM, and they're injected at PreToolUse — a poisoned or prompt-injecting memory body would speak with the system's voice. `block`-kind memories deserve the most scrutiny: their body is delivered alongside a denied tool call, the moment the agent is most likely to comply. Audit what's stored with `engram recall` and watch formation decisions in `engram monitor`.
- **Hints never touch the permission system.** A surfaced `hint` emits only `additionalContext` — no `permissionDecision` — so target approval prompts fire exactly as they would without ToolEngrams. (An explicit hook "allow" would silently bypass the prompt for any command an autonomously-formed trigger matched; the system deliberately never grants that.) Only `block` emits a decision, and it is always `deny`.
- **The watcher's containment boundary is a command allowlist, not trust in the model.** Each watcher role runs through the selected engine with a per-role allowlist — the formation session may only run `engram remember`; the evaluation session only `engram judge` plus `engram quarantine` (id-only, soft-demote, audited with a reason — its emergency brake for demonstrably harmful memories, ADR-0007). A hijacked watcher can write bad memories (auditable, reversible) but can't run arbitrary commands, hard-delete, or rewrite existing bodies.
- **A memory never surfaces into the session that formed it** (hints; ADR-0006). The origin session already lived the episode, a same-session "helpful" would be self-confirmation rather than transfer, and the suppression severs the loop where the watcher re-reads its own injected text as fresh evidence. Blocks are exempt — enforcement fires where the lesson was learned.
...
Harness selection is explicit and mixable:

```bash
./install.sh --target claude-code --engine codex
./install.sh --target claude-code --target codex --engine claude-code
```

`--target` wires hot-path hooks for a coding agent. `--engine` selects the
headless runner for watcher ticks and consolidation. Multiple targets can share
the same DB; one engine is active for background work.

The installer:

1. Installs `toolengrams` (pip editable)
2. Wires target hooks:
   - Claude Code: `~/.claude/settings.json`
   - Codex: enables `[features] hooks = true` in `~/.codex/config.toml` and writes `~/.codex/hooks.json`
   - `SessionStart` (session tracking + idle-sweep), `UserPromptSubmit` (formation tick on correction)
   - `Stop` (formation + eval tick triggers), `SessionEnd`/`PreCompact` or Codex `PreCompact` (flush ticks)
   - `PreToolUse` (block + hint surfacing)
   - `PostToolUse` (turn counter + recovery tick)
   - `PostToolUseFailure` where supported; Codex surfaces failure hints inline on `PostToolUse`
...
Hooks load at session start — **open a new target-agent session** after
installing. The session you ran `./install.sh` from won't have them. For Codex,
...
(Hooks inherit the target process environment — export
...
- Claude Code ≥ 2.1.117 for the `claude-code` target or engine
- Codex ≥ 0.137.0 for the `codex` target or engine (`codex login` handles auth)
...
engram edit <id|name> --body "…"  In-place content correction: preserves id,
                                    counters, surfaces, triggers; stamps
                                    last_verified_ts (--re-extract-triggers opt-in)
engram quarantine <id> --reason   Reversibly pull a harmful memory from
                                    circulation (soft-demote + audit event +
                                    noise-mark; the eval watcher's verb)
...
engram doctor                     Wiring + liveness diagnostics: hooks present,
                                    PATH, target/engine versions, DB, last hook fire.
...
- **watcher_state** — keyed `(work_session_id, role)` (formation + eval): per-role transcript cursor and tick state (`armed`, `last_tick_ts`, `fail_streak`, `target`).
- **watcher_runs** — watcher run history for `engram monitor`, including the active `engine`, model, status, token usage, and cost when reported by the engine.
...
| `ENGRAM_WATCHER_MODEL` | `sonnet` | Model passed to the Claude Code engine for both watcher roles (e.g. `haiku` for a cheaper, faster watcher) |
...
| `ENGRAM_CODEX_WATCHER_MODEL` | unset | Model passed to `codex exec` for both watcher roles; unset lets Codex use `~/.codex/config.toml` defaults |
| `ENGRAM_CODEX_FORMATION_MODEL` | unset | Codex model for the formation role only; beats `ENGRAM_CODEX_WATCHER_MODEL` |
| `ENGRAM_CODEX_EVAL_MODEL` | unset | Codex model for the evaluation role only; beats `ENGRAM_CODEX_WATCHER_MODEL` |
| `ENGRAM_WATCHER_TIMEOUT` | `300` | Per-call engine timeout (seconds) for a watcher tick |
...
- **Destructive-command blocking as the pitch.** The target harness permission system is the right tool for that. Blocks exist as a narrow option, not the headline.
- **Hebbian co-activation.** Removed — recall itself needs to be reliable first.
- **MCP server / arbitrary harnesses.** Claude Code and Codex are the supported harness adapters today.
